...
train_df <- train_df %>%
  mutate(
    `Economic Need Index` = pmin(`Economic Need Index`, 0.95),
    `% Poverty` = pmin(`% Poverty`, 0.95)
  )
...
xgb_recipe <- recipe(`% Chronically Absent` ~ `Economic Need Index` + `% Poverty` + `% English Language Learners` + `% Students with Disabilities`, data = train_df)
...
write_csv(best_params, "data/model_data/xgb/xgb_clipped_best_params.csv")
...
xgb_results <- evaluate_splits(xgb_fit, splits, n_predictors = 4, model_name = "xgb_clipped")
write_csv(xgb_results, "data/model_data/xgb/xgb_clipped.csv")
...
ggsave("plots/xgb_plots/shap__clipped.png", p1)
